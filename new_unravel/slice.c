...
print_slices1 (print_nodes,slices,active,line_num)
	bit_set	slices[],active;
	int		print_nodes;
	int line_num;
{
	int			file,proc;
	int			stmt,count,count1;
	int			line,from,to;
	int			max = 0,f;
	static 		bit_set		map = NULL;
	FILE *fp,*fp1;
	fp = fopen("signature.txt", "w");
	fp1 = fopen("signature_final.txt","a"); 
	...
	for (file = 0; file < n_files; file++){
		count = fprintf (fp1,"%sc %d : \n",files[file].name,line_num);
		stmt = -1;
		if(!print_nodes)clear_bit_set (map);
		while ((stmt = get_next_member(slices[file],stmt)) >= 0){
		//	if (print_nodes)count += printf ("%4d ",stmt);
			 if (print_nodes){count += fprintf (fp,"%4d ",stmt); count1 += fprintf (fp1,"%4d ",stmt);}	 
			else {
		...
		if (!print_nodes){
			...
			while ((line = get_next_member(map,line)) >= 0){
				count += fprintf (fp,"%4d ",line);
				count1 += fprintf (fp1,"%4d ",line);
				if (count > LINE_LENGTH) {
					fprintf (fp,"\n");
					fprintf (fp1,"\n");
					count = 0;
